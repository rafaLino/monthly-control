    ...
    <div className="grid grid-cols-auto sm:grid-cols-4 justify-between px-2 sm:px-4 gap-2 sm:gap-4 items-center h-full text-zinc-600 dark:text-zinc-300 relative">
      {data.map((user) => (
        <Fragment key={user.id}>
          <div className="flex items-center font-semibold group">{user.name}
            <Button variant="link" className='text-red-500 cursor-pointer invisible group-hover:visible' size="icon" onClick={() => onRemove(user.id)}>
